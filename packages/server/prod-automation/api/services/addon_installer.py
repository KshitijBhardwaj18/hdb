    ...
    def _build_argocd_install_script(
        ...
        lines.extend(
            [
                ...
                "",
                "# Configure api_user account for cortex-app integration",
                'echo "==> Configuring ArgoCD api_user account..."',
                """kubectl patch configmap argocd-cm -n argocd --type merge -p '{"data":{"accounts.api_user":"login, apiKey"}}'""",
                """kubectl patch configmap argocd-rbac-cm -n argocd --type merge -p '{"data":{"policy.csv":"p, role:api-manager, applications, *, */*, allow\\np, role:api-manager, repositories, *, *, allow\\ng, api_user, role:api-manager","policy.default":"","policy.matchMode":"glob","scopes":"[groups]"}}'""",
                'echo "==> ArgoCD api_user account configured!"',
                "",
                "# Generate ArgoCD API token for cortex-app (idempotent)",
                'echo "==> Checking for existing ArgoCD API token..."',
                f'EXISTING_TOKEN=$(aws secretsmanager get-secret-value --secret-id /byoc/$CUSTOMER_ID/argocd-generated-tokens --region $REGION --query SecretString --output text 2>/dev/null | jq -r \'.ARGO_CD_API_TOKEN // empty\' || echo "")',
                'if [ -z "$EXISTING_TOKEN" ]; then',
                '    echo "==> Generating ArgoCD API token..."',
                "    kubectl wait --for=condition=available --timeout=120s deployment/argocd-server -n argocd",
                "    API_TOKEN=",
                "    for ATTEMPT in 1 2 3; do",
                '        echo "==> Token generation attempt $ATTEMPT/3..."',
                "        kubectl port-forward svc/argocd-server -n argocd 8080:80 &",
                "        PF_PID=$!",
                "        sleep 10",
                '        for i in $(seq 1 12); do curl -s http://localhost:8080/healthz > /dev/null 2>&1 && break; echo "Waiting for ArgoCD API ($i/12)..."; sleep 5; done',
                """        ARGOCD_ADMIN_PASS=$(kubectl get secret argocd-initial-admin-secret -n argocd -o jsonpath='{.data.password}' | base64 -d)""",
                """        SESSION=$(curl -s -H "Content-Type: application/json" http://localhost:8080/api/v1/session -d "{\\\\"username\\\\":\\\\"admin\\\\",\\\\"password\\\\":\\\\"$ARGOCD_ADMIN_PASS\\\\"}" | jq -r '.token')""",
                '        if [ -n "$SESSION" ] && [ "$SESSION" != "null" ]; then',
                """            API_TOKEN=$(curl -s -H "Content-Type: application/json" http://localhost:8080/api/v1/account/api_user/token -H "Authorization: Bearer $SESSION" -d '{}' | jq -r '.token')""",
                "        fi",
                "        kill $PF_PID 2>/dev/null || true",
                "        wait $PF_PID 2>/dev/null || true",
                "        unset ARGOCD_ADMIN_PASS SESSION PF_PID",
                '        if [ -n "$API_TOKEN" ] && [ "$API_TOKEN" != "null" ]; then',
                '            echo "==> Token generated successfully on attempt $ATTEMPT"',
                "            break",
                "        fi",
                ...
                "    done",
                '    if [ -n "$API_TOKEN" ] && [ "$API_TOKEN" != "null" ]; then',
                f"""        aws secretsmanager create-secret --name /byoc/$CUSTOMER_ID/argocd-generated-tokens --region $REGION --secret-string "$(jq -n --arg token "$API_TOKEN" '{{"ARGO_CD_API_TOKEN": $token}}')" 2>/dev/null || aws secretsmanager put-secret-value --secret-id /byoc/$CUSTOMER_ID/argocd-generated-tokens --region $REGION --secret-string "$(jq -n --arg token "$API_TOKEN" '{{"ARGO_CD_API_TOKEN": $token}}')" """,
                '        echo "==> ArgoCD API token generated and stored in Secrets Manager!"',
                "    else",
                '        echo "==> WARNING: Failed to generate ArgoCD API token after 3 attempts"',
                "    fi",
                "    unset API_TOKEN",
                "else",
                '    echo "==> ArgoCD API token already exists, skipping generation"',
                "fi",
