...
**To add a new tenant:** Create a folder in `infra/tenants/<org-name>/` with `falkor.values.yaml`. ArgoCD automatically
detects and deploys it.
...
**Problem:** Sync waited for stunnel to be healthy, but stunnel needed the TLS cert, which needed the Job to run first.
Deadlock.
